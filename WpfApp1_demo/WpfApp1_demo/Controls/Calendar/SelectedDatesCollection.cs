...
namespace AvePoint.Migrator.Common.Controls
{
    ...
    public sealed class SelectedDatesCollection : ObservableCollection<DateTime>
    {
        ...
        public SelectedDatesCollection(AUICalendar owner)
        {
        ...
        protected override void InsertItem(int index, DateTime item)
        {
            ...
            if (!this.Contains(item))
            {
                ...
                if (AUICalendar.IsValidDateSelection(this._owner, item))
                {
        ...
        protected override void SetItem(int index, DateTime item)
        {
            ...
            if (!this.Contains(item))
            {
                ...
                {
                    if (item != null && DateTime.Compare(this[index], item) != 0 && AUICalendar.IsValidDateSelection(this._owner, item))
                    {
                        removedItems.Add(this[index]);
                        base.SetItem(index, item);
                        UpdateMinMax(item);
        ...
        internal void AddRangeInternal(DateTime start, DateTime end)
        {
            ...
            foreach (DateTime current in GetDaysInRange(start, end))
            {
                if (AUICalendar.IsValidDateSelection(this._owner, current))
                {
                    this.Add(current);
        ...
        {
            if (AUICalendar.IsValidDateSelection(this._owner, date))
            {
        ...
        {
            this._owner.OnSelectedDatesCollectionChanged(new CalendarSelectionChangedEventArgs(AUICalendar.SelectedDatesChangedEvent, removedItems, addedItems));
        }
