...
{
    [TemplatePart(Name = AUICalendar.ElementRoot, Type = typeof(Panel))]
    [TemplatePart(Name = AUICalendar.ElementMonth, Type = typeof(AUICalendarItem))]
    public class AUICalendar : Control
    {
        #region Constants
        ...
        private AUICalendarItem _monthControl;
        ...
        public static readonly RoutedEvent SelectedDatesChangedEvent = EventManager.RegisterRoutedEvent("SelectedDatesChanged", RoutingStrategy.Direct, typeof(EventHandler<SelectionChangedEventArgs>), typeof(AUICalendar));
        ...
        static AUICalendar()
        {
            DefaultStyleKeyProperty.OverrideMetadata(typeof(AUICalendar), new FrameworkPropertyMetadata(typeof(AUICalendar)));
            KeyboardNavigation.TabNavigationProperty.OverrideMetadata(typeof(AUICalendar), new FrameworkPropertyMetadata(KeyboardNavigationMode.Once));
            KeyboardNavigation.DirectionalNavigationProperty.OverrideMetadata(typeof(AUICalendar), new FrameworkPropertyMetadata(KeyboardNavigationMode.Contained));

            EventManager.RegisterClassHandler(typeof(AUICalendar), UIElement.GotFocusEvent, new RoutedEventHandler(OnGotFocus));
            LanguageProperty.OverrideMetadata(typeof(AUICalendar), new FrameworkPropertyMetadata(new PropertyChangedCallback(OnLanguageChanged)));
        }
        ...
        {
            AUICalendar c = d as AUICalendar;
            Debug.Assert(c != null);
        ...
        private static void OnDisplayModePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            AUICalendarItem monthControl = c.MonthControl;
        ...
        internal AUICalendarItem MonthControl
        {
        ...
        public override void OnApplyTemplate()
        {
            ...
            _monthControl = GetTemplateChild(ElementMonth) as AUICalendarItem;
            _monthControl.InitCompleted -= _monthControl_InitCompleted;
            _monthControl.InitCompleted += _monthControl_InitCompleted;
        ...
        AUITimePicker hourMinutePicker;

        void hourMinutePicker_SelectedTimeChanged(object sender, SelectionChangedEventArgs e)
        {
            DateTime? date = this.SelectedDate;
            DateTime time = this.hourMinutePicker.SelectedTime;
            if (date.HasValue)
            {
                this.SelectedDate = new DateTime(date.Value.Year, date.Value.Month, date.Value.Day, time.Hour, time.Minute, 0);
            }
        }

        void _monthControl_InitCompleted()
        {
            AUITimeZonePicker = _monthControl.AUITimeZonePicker;
            AUITimePicker = _monthControl.AUITimePicker;
        ...
        internal AUICalendarDayButton FindDayButtonFromDay(DateTime day)
        {
            if (this.MonthControl != null)
            {
                foreach (AUICalendarDayButton b in this.MonthControl.GetCalendarDayButtons())
                {
        ...
        internal static bool IsValidDateSelection(AUICalendar cal, object value)
        {
        ...
        internal void OnCalendarButtonPressed(AUICalendarButton b, bool switchDisplayMode)
        {
            if (b.DataContext is DateTime)
            {
        ...
        {
            AUICalendarItem monthControl = this.MonthControl;
        ...
        private static bool IsValidKeyboardSelection(AUICalendar cal, object value)
        {
            if (value == null)
            {
                return true;
        ...
        private bool ProcessCalendarKey(KeyEventArgs e)
        {
            if (this.DisplayMode == CalendarMode.Month)
            {
                ...
                AUICalendarDayButton currentDayButton = (MonthControl != null) ? MonthControl.GetCalendarDayButton(this.CurrentDate) : null;
