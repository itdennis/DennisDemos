...
namespace AvePoint.Migrator.Common.Controls
{
    ...
    public class DataGrid : MultiSelector
    {
        ...
        // expand by Taven
        private const string DATAGRID_elementColumnsIndicator = "ColumnsIndicator";
        #endregion
        ...
        public static readonly DependencyProperty AutoGenerateColumnsProperty =
            DependencyProperty.Register("AutoGenerateColumns", typeof(bool), typeof(DataGrid), new FrameworkPropertyMetadata(false, new PropertyChangedCallback(OnAutoGenerateColumnsPropertyChanged)));
        ...
        /// <summary>
        /// ��ȡ������<see cref="AUIPager"/>�����ʵ����
        /// <seealso cref="AvePoint.Migrator.Common.Controls.AUIPager"/>
        /// </summary>
        public Pager Pager
        {
            get { return (Pager)GetValue(PagerProperty); }
            set { SetValue(PagerProperty, value); }
        }

        // Using a DependencyProperty as the backing store for Pager.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty PagerProperty =
            DependencyProperty.Register("Pager", typeof(Pager), typeof(DataGrid), new PropertyMetadata(null, OnPagerChanged));

        private static void OnPagerChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            var self = d as DataGrid;
            var pager = e.NewValue as Pager;
            self.ProcessPager(pager);
        }
        private void ProcessPager(Pager pager)
        {
            this.ResetPagerCount(pager);
            this.Sorting += DataGrid_Sorting;
            pager.PageIndexChanged += pager_PageIndexChanged;
            pager.PageSizeChanged += pager_PageSizeChanged;
            var view = this.ItemsSource as PagedCollectionView;
            if (view != null)
            {
                view.CollectionChanged += (s, e) =>
                {
                    if (view.NeedsRefresh && !view.IsEmpty)
                    {
                        view.Refresh();
                    }
                };
                view.OnItemCountChanged += (count) =>
                {
                    this.Pager.PageCount = count;
                    this.Pager.PageIndexChanged -= pager_PageIndexChanged;
        ...
        {
            this.ResetPagerCount(this.Pager);
            var view = this.ItemsSource as PagedCollectionView;
            if (view != null)
            {
                view.PageSize = this.Pager.PageSize;
                view.MoveToPage(this.Pager.PageIndex - 1);
                view.Refresh();
            }
        }
        ...
        private void ResetPagerCount(Pager pager)
        {
            var pageSize = pager.PageSize;
            var view = this.ItemsSource as PagedCollectionView;
            if (view != null)
            {
                var total = view.TotalItemCount;
                var count = total / pageSize + (total % pageSize == 0 ? 0 : 1);
                pager.PageCount = count == 0 ? 1 : count;
            }
        }

        private void OnItemsSourceChangedAndUpdatePagingCollectionView()
        {
        }
        #endregion
    }
